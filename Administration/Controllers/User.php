...
class User extends MainController
{
    ...
    public function indexAction()
    {
        ...
        $this->addDataView(["viewTitle" => "Admin - Users", "user" => $result]);
    }
    ...
    public function modifyAction()
    {
        ...
        if (!empty($_POST['nickname']) && !empty($_POST['mail'])) {
            ...
            if (!empty($nicknameResult) && $_POST['nickname'] != $form->nickname) {
                $alert = Tools\Alert::render("Ce nom utilisateur existe déjà !",
                  "danger");
            } elseif (!filter_var($_POST['mail'], FILTER_VALIDATE_EMAIL)) {
                $alert = Tools\Alert::render("le format de l'adresse mail n'est pas bon !",
                  "danger");
            } elseif (!empty($mailResult) && $_POST['mail'] != $form->mail) {
                $alert = Tools\Alert::render("Cette adresse mail existe déjà !",
                  "danger");
            } elseif ($_POST['passwordConfirm'] != $_POST['password']) {
                $alert = Tools\Alert::render("Les deux mot de passes ne concorde pas",
                  "danger");
            } else {
                ...
                $alert = Tools\Alert::render("L'utilisateur " . $_POST['nickname'] . " a bien été modifier ! ",
                  "success");
                $action = true;
            }
        }
    ...
    public function addAction()
    {
        ...
        if (!empty($_POST['nickname']) && !empty($_POST['mail']) && !empty($_POST['password']) && !empty($_POST['passwordConfirm'])) {
            ...
            if (!empty($nicknameResult)) {
                ...
                $alert = Tools\Alert::render("le mote passe est obligatoire !",
                  "danger");
            } elseif (empty($_POST['passwordConfirm'])) {
                $alert = Tools\Alert::render("La confirmation du mot de passe est obligatoire !",
                  "danger");
            } elseif ($_POST['passwordConfirm'] != $_POST['password']) {
                $alert = Tools\Alert::render("Les deux mot de passes ne concorde pas",
                  "danger");
                ...
                $alert = Tools\Alert::render("L'utilisateur " . $_POST['nickname'] . " a bien été ajouter! ",
                  "success");
            }
        }
    ...
    public function viewAction()
    {
        ...
        $user = $user[0];
        $this->addDataView(["viewTitle" => "Admin - Users", "user" => $user]);
    }
}
